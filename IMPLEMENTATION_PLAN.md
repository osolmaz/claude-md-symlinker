...
claudemdeez apply
claudemdeez apply ~/repos --dry-run
claudemdeez watch
claudemdeez doctor
claudemdeez clean --dry-run
```
...
`~/.config/systemd/user/`, run `claudemdeez watch`, and never require root.
...
reconciliation. If a timer is added, it must call `claudemdeez apply` using the
...
```sh
cargo test
cargo run -- apply ~/repos/claudemdeez-fixtures --dry-run
cargo run -- apply ~/repos/claudemdeez-fixtures
cargo run -- doctor
...
- Generate a `systemd --user` unit for `claudemdeez watch`.
...
1. `claudemdeez apply <root>` safely reconciles real Git repositories.
...
1. `claudemdeez service install` creates a valid user systemd unit.
